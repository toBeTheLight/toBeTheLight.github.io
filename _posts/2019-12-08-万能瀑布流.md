...
对于内部有图片异步加载的情况，通常的做法是写死图片高度，或检测内部的 img 元素从而在 onload 事件中进行重排。

由于我们业务中子元素存在本身异步初始化、内部数据异步获取等其他引起尺寸变化的情况，且这种尺寸变化时机不可确定，为满足这种需求所以调研完成了一个通用万能的瀑布流布局实现。
...
为了便于后续对不同类型的块可传入不同类型的配置，拆分设计为 Waterfall.vue 瀑布流容器和 WaterfallItem.vue 瀑布流块两个组件。
...
为了保证在布局块在新增或删除时能使重新布局的成本最小化，我选择由 WaterfallItem.vue 告知 Waterfall.vue 自己的新增和移除。
...
至此，基本的瀑布流逻辑也就结束了，[使用现代浏览器点此预览](https://tobethelight.github.io/demo/layout/waterfall/simple/index.html)
...
至此，可动态感知尺寸变化的瀑布流也就基本完成了，[使用现代浏览器点此预览](https://tobethelight.github.io/demo/layout/waterfall/powerful/index.html)

预览中定时修改部分 WaterfallItem 的字体大小。

## 优化

在以上实现之外还可以做一些其他优化，如：

1. 通知 Waterfall.vue 添加的 add 和更新的 update 方法调用有充分触发的情况，可以合并。
...
5. 对应 keep-alive 的路由渲染，在 activated 和 deactivated 中进行重新排列的停止和激活，避免错误和不必要的开支。